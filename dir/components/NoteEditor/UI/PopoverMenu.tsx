  ...
  return (
    <Popover.Root onOpenChange={onOpenChange}>
      {customTrigger ? (
        ...
        <Trigger asChild>
          <ActionBar.Button className={triggerClassName} tooltip={!isOpen ? tooltip : ''}>
            {trigger}
          </ActionBar.Button>
        </Trigger>
      )}
      {withPortal ? (        
        <Popover.Portal >
          <Popover.Content asChild sideOffset={8}>
            <Container className="min-w-[15rem] p-2 flex flex-col gap-0.5 max-h-80 overflow-auto z-[9999]">
              {children}
            </Container>
          </Popover.Content>
        </Popover.Portal>
